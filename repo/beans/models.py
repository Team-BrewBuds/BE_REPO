...
class OfficialBean(models.Model):

    bean_type_choices = [("single", "싱글 오리진"), ("blend", "블랜드")]
    roasting_point_choices = list(zip(range(0, 6), range(0, 6)))  # 0~5

    # 원두 맛
    bean_taste = models.OneToOneField(
        "BeanTaste", on_delete=models.CASCADE, null=True, blank=True, related_name="official_bean", verbose_name="원두 맛"
    )

    # 원두 공식 사진
    image_url = models.ImageField(upload_to="beans/images/", null=True, blank=True, verbose_name="공식원두 이미지")
