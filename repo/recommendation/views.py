...
class BeanRecommendAPIView(APIView):
    ...
    def get(self, request, user_id=None):
        ...
        user = CustomUser.objects.get(id=user_id) or request.user

        strategy = BeanRecommendationStrategy(user)
        service = RecommendationService(strategy)
        recommend_beans = service.recommend()

        serializer = BeanRecommendSerializer(recommend_beans, many=True)
