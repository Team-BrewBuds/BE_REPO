...
from repo.search.serializers import BuddySearchSerializer, BeanSearchSerializer, TastedRecordSearchSerializer, PostSearchSerializer 
...
class BuddySearchView(APIView):
    ...
    def get(self, request):
        ...
        users = CustomUser.objects.filter(nickname__icontains=query).annotate(
            record_cnt=Count('tastedrecord'),
            follower_cnt=Count('relationships_to', filter=Q(relationships_to__relationship_type="follow"))
        )
...
class BeanSearchView(APIView):
    ...
    def get(self, request):
        ...
        beans = Bean.objects.filter(name__icontains=query).annotate(
            avg_star=Avg('tastedrecord__taste_review__star'),
            record_cnt=Count('tastedrecord')
        )
