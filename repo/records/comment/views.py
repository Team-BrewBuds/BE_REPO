...
from repo.common.permissions import IsAuthorOrOwner, IsOwnerOrReadOnly
...
class CommentDetailAPIView(APIView):
    ...
    def get_permissions(self):
        return [IsAuthorOrOwner()] if self.request.method == "DELETE" else [IsOwnerOrReadOnly()]
    ...
    def patch(self, request, id):
        ...
        comment = self.get_object(id)
        updated_comment = self.comment_service.update_comment(comment, valid_serializer.validated_data)

        response_serializer = CommentOutputSerializer(updated_comment, context={"request": request})
        return Response(response_serializer.data, status=status.HTTP_200_OK)
